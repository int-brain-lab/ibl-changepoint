function [nLL,output] = changepoint_bayesianold_nll(params,data,pflag)
%CHANGEPOINT_BAYESIAN_NLL Bayesian online changepoint detection observer.
%
% WARNING: This version contains a bug! Use the new version.
%
...
if numel(sessions) > 1
    ...
    for iSession = 1:numel(sessions)
        ...
        if nargout > 1
            [nLL_temp,output1] = changepoint_bayesianold_nll(params,data1,pflag);
            if isempty(output)
                output = output1;
            else
            ...
            nLL_temp = changepoint_bayesianold_nll(params,data1,pflag);
        end
        nLL = [nLL; nLL_temp];
    end
    return;
